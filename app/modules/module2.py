...
def run():
    st.header("Análise dos pontos mais instáveis")

    top_n = st.slider("Número de pontos mais instáveis", 1, 10, 1)
    ...
    for _, row in df_top.iterrows():
        ...
        gdf_all.plot(ax=ax_map, column='up', cmap=plt.cm.jet, markersize=5, legend=True, cax=cax)
        ...
        ax_corr.set_title(f'Correlação up vs Nível')
